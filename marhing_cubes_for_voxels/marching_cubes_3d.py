...
from meshobjs import Node, Mesh, Face
...
def marching_cubes_3d_single_cell(f_eval, x, y, z, el):
    ...
    def edge_to_boundary_vertex(edge):
        ...
        return Node(x + ((vert_pos0[0]) * t0 + (vert_pos1[0]) * t1 - 0.5) * el,
    ...
    for face in faces:
        ...
        for n in verts:
            m.add_node(n)
        m.add_face(Face([m.nodes[nvi], m.nodes[nvi+1], m.nodes[nvi+2]]))
    return m

def original_marching_cubes_3d_single_cell(f, x, y, z, el):
    # Evaluate f on each vertex of the cube
    # el (edge length) addition by mathias bernhard
    f_eval = [None] * 8
    for v in range(8):
